 ...
 */
class Klaim extends CActiveRecord
{
	/**
    ...
    {
        $_first = "001";
        $last_po = $this->model()->find(array('order'=>'id DESC'));
        $no = $_first; 
        if($last_po != null){
			$_no = $last_po->id;
            $_no++;
	...
	public function rules()
	{
		...
		return array(
			array('id, field', 'required'),
			array('url', 'length', 'max'=>120),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, field, url ', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return Klaim the static model class
	 */
